...
class Cluster:
    ...
    def recalculate_center(self):
        ...
        if np.equal(center, self.center).all():
            self.changed = False
        # print(eucl_dist(self.center, center))
        self.center = center


def kmeans(data, k, distance_function, max_iter=100):
    cluters = []
...
if __name__ == '__main__':
    ...
    labels, clusters = kmeans(data, 16, distance)
    # print(labels)
    ...
    labels, clusters = kmeans(data, 16, eucl_dist1)
    # print(labels)

    print(adjusted_mutual_info_score(true_labels, labels))

    print("Starting k means with eucl_dist1 k = 2")
    labels, clusters = kmeans(data, 16, eucl_dist)
    # print(labels)

    print(adjusted_mutual_info_score(true_labels, labels))

    print("Starting k means with cos")
    labels, clusters = kmeans(data, 16, cos_dist)
    # print(labels)
    ...
    labels, clusters = kmeans(data, 16, distance1)
    # print(labels)
    print(adjusted_mutual_info_score(true_labels, labels))
